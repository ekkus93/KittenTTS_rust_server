...
The server keeps the same high-level separation of concerns as the Python shim,
but with explicit Rust module boundaries:

- `routes/` owns HTTP parsing, response shaping, and route-specific error style
- `middleware/` owns auth policy, request IDs, and request-scoped metadata
- `services/voices.rs` owns compatibility-sensitive voice resolution and voice-list shaping
- `services/audio.rs` owns PCM conversion, normalization, and WAV/PCM serialization
- `backend/kitten.rs` owns the `kitten_tts_rs` integration, backend asset checks, ONNX Runtime discovery, and `espeak-ng` verification
- `app_state.rs` wires validated settings, engine metadata, and the active synthesis runtime into the Axum app

Compatibility-sensitive behavior is intentionally kept near the HTTP boundary
when it affects client-visible semantics, especially for voice resolution,
error envelopes, output-format negotiation, and pseudo-streaming.
...
- the sibling `kitten_tts_rs` crate as the synthesis backend
- `espeak-ng` for phonemization
- ONNX Runtime as a shared library, either through `ORT_DYLIB_PATH`, local discovery, or a system install
- KittenTTS model assets: `config.json`, the referenced ONNX model file, and `voices.npz`

Asset resolution follows two modes:

- local model directory mode: set `model_dir` or `KITTENTTS_SERVER_MODEL_DIR` and provide the required files directly
- Hugging Face mode: leave `model_dir` unset and the runtime will follow the Python-compatible download/cache path for the default model repo

The server treats missing model assets, missing ONNX Runtime, and missing
`espeak-ng` as startup-fatal conditions in the real backend path.
...
synthesis in v1. It preserves the Python shim's current pseudo-streaming model:

- synthesize the full audio payload first
- normalize and serialize the final bytes
- emit those already-generated bytes as a streamed HTTP body

That means pseudo-streaming improves response delivery semantics for existing
clients, but it does not reduce backend synthesis latency or memory usage the
way a true incremental TTS pipeline would.

## Non-goals

The current Rust port is intentionally compatibility-first and does not expand
the scope beyond the Python shim's current contract. Non-goals for this version
include:

- true incremental synthesis
- additional output codecs such as MP3, AAC, or Opus
- SSML support
- voice cloning or remote voice providers
- persistence layers such as databases or queues
- redesigning public routes or replacing the current auth/error envelopes with a new API shape
